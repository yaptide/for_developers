...
Then run the following command from the `ui/` directory:

```bash
docker compose up
```

Open **http://localhost:3000** and log in using credentials you created (username: `admin`, password: `password`). The setup is complete now.
